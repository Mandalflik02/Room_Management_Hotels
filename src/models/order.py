class Order():
	...
	def __str__(self):
		...
		with_food = str(str(str("Order with food: %s" % (self.with_food))).ljust(len1 + 5)) + "||"
		arrivel = str(str(str("Arrivel date: %s" % (self.arrivel_date))).ljust(len1 + 5)) + "||"
		leaving = str(str(str("Customer name: %s" % (self.leaving_date))).ljust(len1 + 5)) + "||"
		return (
				f"\n{'=' * (len1 + 7)}\n%s\n%s\n%s\n%s\n%s\n%s\n{'=' * (len1 + 7)}\n" % (
			order_num, customer_name, number_of_guests, with_food, arrivel, leaving)
		)
