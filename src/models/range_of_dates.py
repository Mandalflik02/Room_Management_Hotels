...
class Dates_Range():
	...
	def is_date(self, string):
		...
		if re.fullmatch(regex, date) != None:  # check the string is in date format
			...
			if (check_date_numbers [ 0 ] > 31 or check_date_numbers [ 0 ] < 1) or (
				...
				# check the day number is between 1 to 31 and the month number is between 1 to 12
				return ""
			return date
		return ""
	...
	def check_range(self, start, end):
		arrivel = self.conver_to_int(start.split("/"))  # conver to list with number not strings
		leaving = self.conver_to_int(end.split("/"))  # conver to list with number not strings
		# 30/10/2020
		# 1/11/2020
		if arrivel [ 2 ] == leaving [ 2 ]:
			# check range in the same yaer
			count = leaving [ 1 ] - arrivel [ 1 ]
			if count > 2:
				# the range can by more then 2 month
			...
			# check range when the yaer is different
			yaer_count = leaving [ 2 ] - arrivel [ 2 ]
			month_count = (12 - arrivel [ 1 ]) + leaving [ 1 ]
			# the range can by more then 2 month and certainly not more than two years
			if yaer_count > 1:
