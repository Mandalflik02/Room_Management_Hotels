...
class Room_View_Widget(QWidget):
	...
	def create_room_frame(self,room=None | Room ):
		...
		dates_catch_room.setText(str(room.get_dates_catch()))
		...
		delete_room_btn.clicked.connect(lambda: self.delete_room(room.get_room_number()))

		return room_frame

	def delete_room(self,room_to_delete):
		delete_status=MSG_Dialog(f"Delete room number {room_to_delete}", "Yes", "No")
		delete_status.exec()
		print(delete_status.status)
		if delete_status.status == "Yes" :
			room_index=-1
			for i in range(len(ROOMS)):
				if ROOMS[i].get_room_number() == room_to_delete:
					room_index = i
					break
			if room_index != -1 and room_index in range(len(ROOMS)):
				ROOMS.pop(room_index)
			print(f"Delete room:{room_to_delete}")
			# for r in ROOMS:
			# 	print(r)
			print("\n")
